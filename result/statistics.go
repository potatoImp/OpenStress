...
func (c *Collector) CalculateAvgResponseTime(results []ResultData) ([]float64, []float64, []float64, int64, int64) {
	// 按秒聚合数据
	totalResponseTime := make(map[int64]int64)   // 每秒的总响应时间
	successResponseTime := make(map[int64]int64) // 每秒的成功请求的响应时间
	failureResponseTime := make(map[int64]int64) // 每秒的失败请求的响应时间
...
func (c *Collector) CalculateAvgTraffic(results []ResultData) ([]float64, []float64, []float64, int64, int64) {
	// 按秒聚合数据
	...
	var avgSentTraffic []float64
	var avgReceivedTraffic []float64
	var avgSuccessSentTraffic []float64
	var avgSuccessReceivedTraffic []float64

	for _, sec := range xAxis {
		// 计算每秒的平均发送流量
		if successCount[sec]+failureCount[sec] > 0 {
			avgSentTraffic = append(avgSentTraffic, float64(totalSent[sec])/float64(successCount[sec]+failureCount[sec]))
			avgReceivedTraffic = append(avgReceivedTraffic, float64(totalReceived[sec])/float64(successCount[sec]+failureCount[sec]))
		} else {
		...
		// 计算每秒成功请求的平均发送流量
		if successCount[sec] > 0 {
			avgSuccessSentTraffic = append(avgSuccessSentTraffic, float64(successSent[sec])/float64(successCount[sec]))
			avgSuccessReceivedTraffic = append(avgSuccessReceivedTraffic, float64(successReceived[sec])/float64(successCount[sec]))
		} else {
