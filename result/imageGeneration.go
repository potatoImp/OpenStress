...
func GenerateFlowTrendChartAsync(avgSentTrafficValues, avgReceivedTrafficValues []int, avgTrafficStartTime int64, avgTrafficEndTime int64, dir string) (string, error) {
	...
	// 调整横坐标点数
	xAxis, avgSentTrafficValues := adjustXAxisPoints(startTimeTime, endTimeTime, avgSentTrafficValues)
	xAxis, avgReceivedTrafficValues = adjustXAxisPoints(startTimeTime, endTimeTime, avgReceivedTrafficValues)

	// 创建折线图对象
	line := charts.NewLine()

	// 添加数据系列
	line.AddSeries("Sent Traffic", generateLineData(avgSentTrafficValues))
	line.AddSeries("Received Traffic", generateLineData(avgReceivedTrafficValues))
	...
	// 设置全局选项
	line.SetGlobalOptions(charts.WithTitleOpts(opts.Title{
		Title:    "Flow Trend Over Time",
		Subtitle: fmt.Sprintf("Test Duration: %s to %s", startTimeTime.Format("15:04:05"), endTimeTime.Format("15:04:05")),
	}))

	// 保存为 SVG 文件
	svgFilePath := filepath.Join(dir, "flow_trend_chart.svg")
	svgFile, err := os.Create(svgFilePath)
	if err != nil {
		return "", fmt.Errorf("failed to create SVG file: %v", err)
	}
	defer svgFile.Close()

	err = line.Render(svgFile)
	if err != nil {
